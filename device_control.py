...
def reset_to_original_values(window, window_offset_y, window_offset_x, window_height, window_width, pad_pos):
    ...
    for i, (bdf, original_value) in enumerate(original_values.items()):
        try:
            ...
            pad_pos = functions.progress_bar(i + 1, total_bdfs, 'Resetting Original Values', 'Complete', 1, window_width-50, '█', window, window_offset_y, window_offset_x, window_height, window_width, pad_pos)
        except Exception as e:
